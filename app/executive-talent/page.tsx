...
const countryFlags = [
  { name:"Côte d'Ivoire", flag:<svg width="36" height="24" viewBox="0 0 36 24" style={{borderRadius:2,display:'block'}}><rect width="12" height="24" fill="#F77F00"/><rect x="12" width="12" height="24" fill="#fff"/><rect x="24" width="12" height="24" fill="#009A44"/></svg> },
      ...
      <section style={{position:'relative',minHeight:'80vh',display:'flex',flexDirection:'column',justifyContent:'flex-end',overflow:'hidden'}}>
        ...
        <div style={{position:'relative',zIndex:2,maxWidth:1200,margin:'0 auto',padding:'0 48px 88px',width:'100%'}}>
          <div style={{fontFamily:"'Montserrat',sans-serif",fontSize:11,fontWeight:700,letterSpacing:'0.22em',textTransform:'uppercase',color:'#C9A044',display:'flex',alignItems:'center',gap:12,marginBottom:22}}>
            <span className="hero-eyebrow-line"/>Executive Talent
          </div>
          <h1 style={{fontFamily:"'Montserrat',sans-serif",fontSize:'clamp(48px,7.5vw,96px)',fontWeight:700,color:'#fff',lineHeight:0.96,letterSpacing:'-0.03em',marginBottom:20}}>
            <span className="hero-word">Hire leaders</span>{' '}
            <span className="hero-word delay-1">who can win</span>{' '}
            <span className="hero-word delay-2">in Africa.</span>
          </h1>
          <p className="hero-sub-anim" style={{fontSize:19,fontWeight:300,color:'rgba(255,255,255,0.6)',lineHeight:1.75,maxWidth:520,marginBottom:36}}>
          ...
          <div className="reveal">
            ...
            <div style={{display:'flex',alignItems:'center',gap:36,flexWrap:'wrap'}}>
              {['/clients/alexander-tax-law.webp','/clients/doss-law.png','/clients/rdl-consulting.webp'].map((src,i)=>(
                <Image key={i} src={src} alt="" width={120} height={28} className="client-logo" style={{height:28,width:'auto',objectFit:'contain',filter:'grayscale(30%)',opacity:0.7}}/>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="section">
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:80,alignItems:'center'}}>
            <div className="reveal-left">
              ...
              <span style={{display:'block',fontFamily:"'Montserrat',sans-serif",fontSize:'clamp(32px,5vw,56px)',fontWeight:700,color:'var(--red)',lineHeight:1.05,margin:'16px 0 12px',letterSpacing:'-0.02em'}}>
                So when we bring someone,<br/>it&apos;s not a guess.
              </span>
            </div>
            <div className="reveal-right" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,height:480}}>
